...
/*    private float lastSpeed;
    ...
    public void setSpeed(float speed) {
        if (speed != lastSpeed) {
            lastSpeed = speed;
            sendWriteOneData(ParamCons.CMD_SET_SPEED, DataTypeConversion.shortToBytes((short) (speed * SpManager.getRpmRate())));
        }
    }
    ...
    public void calibrateSpeedByRpm(float speed, int rpm) {
        sendWriteOneData(ParamCons.CMD_SET_SPEED, DataTypeConversion.shortToBytes((short) (speed * rpm)));
    }

    @Override
    public void setLube(int onOff) {
        if (ErrorManager.getInstance().isSafeError) {
            sendUnClearPackage(SerialCommand.TX_WR_ONE, ParamCons.CMD_LUBE, new byte[]{(byte) onOff, 0x00});
        } else {
            sendWriteOneData(ParamCons.CMD_LUBE, new byte[]{(byte) onOff, 0x00});
        }
    }
    ...
    public void writeDeviceType() {
        sendWriteOneData(ParamCons.PARAM_DEVICE, new byte[]{(byte) 0xAA});
    }
