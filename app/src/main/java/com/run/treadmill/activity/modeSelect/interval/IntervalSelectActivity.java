...
import com.run.treadmill.activity.runMode.interval.IntervalActivity;
 ...
 * @Time 2019/08/21
 */
@CreatePresenter(IntervalSelectPresenter.class)
public class IntervalSelectActivity extends BaseSelectActivity<IntervalSelectView, IntervalSelectPresenter> implements IntervalSelectView, CalculatorCallBack, RadioGroup.OnCheckedChangeListener {
    ...
    private void init() {
        tv_title.setText(StringUtil.valueAndIcon(this, getString(R.string.string_mode_interval), R.drawable.img_program_interval_icon));

        mCalcBuilder = new BaseCalculator.Builder(new CalculatorOfSelectMode(this));

        UserDB userInfo = UserInfoManager.getInstance().getUserInfo(CTConstant.INTERVAL);
    ...
    protected int getLayoutId() {
        return R.layout.activity_select_interval;
    }
        ...
        switch (view.getId()) {
            ...
            case R.id.btn_start:
                ...
                getPresenter().setRunningParam(
                        ...
                        rg_gender_info.getCheckedRadioButtonId() == R.id.rb_male
                                ? InitParam.DEFAULT_GENDER_MALE : InitParam.DEFAULT_GENDER_FEMALE
                );
